...
YOLOE-26 is a good MVP detector, not the whole solution. It can detect individual people in group entry and preserve low-confidence partial-occlusion boxes, but entry/exit, re-entry, staff exclusion, queue abandonment, and conversion require application logic. The current submission implements those with entry-line direction checks, staff role heuristics, POS-aware abandonment enrichment, and session stitching. The highest-value next model improvement is a small crop classifier for staff/customer role, trained on labeled crops from the provided footage. That would be more reliable than pure color heuristics while staying privacy-preserving because it classifies role, not identity.
